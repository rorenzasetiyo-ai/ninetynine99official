...
function addToCart() {
  ...
  if (existing) { existing.qty += AppState.currentQty; }
  else {
    const imgEl = el(AppState.currentProduct.imgMainId);
    AppState.cart.push({
  ...
  t.style.background = '#f0faf4';
  t.style.borderColor = '#b2dfcb';
  t.style.color = '#2d7a4f';
  setTimeout(() => t.style.display = 'none', 2000);
  setTimeout(() => closeProductDetail(), 2000);
}
